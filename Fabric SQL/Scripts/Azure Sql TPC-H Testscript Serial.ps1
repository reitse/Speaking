...
$azureInstance = "[].database.windows.net"
$azureDatabase = "[]"
$server = Connect-DbaInstance -SqlInstance $azureInstance -Database $azureDatabase -SqlCredential $azureCredential
Write-Host "Connected to $server with credential $azureCredential" -ForegroundColor Magenta

#$server = Connect-DbaInstance -SqlInstance [].database.fabric.microsoft.com -Database "[]" -DisableException
$sku = 'AzureLaptop'
$edition = 'SQL'
